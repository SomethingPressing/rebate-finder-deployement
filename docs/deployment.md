...
**`scripts/provision.sh`** creates the DigitalOcean Droplet, runs bootstrap, seeds the database, and creates the first admin user — entirely from your laptop. No SSH session or GitHub console interaction needed.

```bash
export DO_API_TOKEN=dop_v1_...      # DigitalOcean API token
export GITHUB_PAT=ghp_...           # GitHub PAT with repo scope — auto-registers deploy keys
export OPENAI_API_KEY=sk-...        # Required for AI features
export APP_DOMAIN=client.incenva.com
export DO_SSH_KEY_IDS=12345678      # Your personal DO SSH key ID (optional but recommended)

# Optional — defaults shown:
# export DO_REGION=nyc1
# export DO_SIZE=s-2vcpu-4gb
# export ADMIN_EMAIL=admin@incenva.com
# export ADMIN_PASSWORD=<auto-generated>
...
| Step | Action |
|------|--------|
| 1 | Creates a temporary ED25519 SSH key, registers it with DigitalOcean API |
| 2 | Creates the Droplet (Ubuntu 22.04, 2 vCPU / 4 GB by default) |
| 3 | Polls until Droplet status = active with a public IP |
| 4 | Waits for SSH port 22 to be reachable |
| 5 | SSH-runs `bootstrap.sh` with `GITHUB_PAT` set — deploys keys automatically, no pause |
| 6 | Injects `OPENAI_API_KEY` and optional env vars into `.env` |
| 7 | Runs `seed.sh` to load filter groups, categories, portfolios, segments |
| 8 | Runs `create-admin.sh` to create the first admin user |
| 9 | Health-checks the app URL (HTTP 200/30x = success) |
| 10 | Removes the temporary SSH key from DigitalOcean |
...
| 10 | Go scraper: build and start binaries |
